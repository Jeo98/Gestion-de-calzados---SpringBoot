...
public class Conexion {
    ...
    public void insertarCalzado(Calzado calzado){
        try {
            conectar();
            ps = conectar.prepareStatement("INSERT INTO Calzado (codigo,descripcion,marca,precioCosto,precioVenta,color,talle,stock) VALUES (?,?,?,?,?,?,?,?)");
            ...
            ps.setString(6, calzado.getColor());
            ps.setFloat(7, ((Float)  (calzado.getTalle())));
            ps.setInt(8, calzado.getStock());
    ...
    //----> HACER INSERTAR DE CLASES PRINCIPALES RESTANTES  \\ MENOS CLASE RENGLON

    // ----------------------BUSCAR----------------------------------------------
    
    public ResultSet buscarCalzadoPorCodigo(String codigocalzado) {

        try {
            conectar();
            ps = conectar.prepareStatement("SELECT * FROM Calzado WHERE codigo = ?");
            ps.setString(1, codigocalzado);
        ...
        return resultado; //deberia retornar Tabla Calzado
    }

    public ResultSet buscarEmpleadoPorLegajo( int legajo) {
        try {
            conectar();
            ps = conectar.prepareStatement("SELECT * FROM Empleado WHERE legajo = ?");
            ps.setInt(1, legajo); // esto hace matching con el signo de pregunta de la query
            resultado = ps.executeQuery();
    ...
    public ResultSet buscarClientePorDNI(Integer numeroDocumento) {
        //ATENCION...
        try {
            conectar();
            ps = conectar.prepareStatement("SELECT * FROM Cliente WHERE numeroDocumento = ?");
            ps.setInt(1, numeroDocumento); //el tipo de dato es INTEGER, pero no puedo hacer ps.setInteger... que hacemos?
    ...
    public ResultSet buscarSucursal( String nombreSucursal) {
        try {
            conectar();
            ps = conectar.prepareStatement("SELECT * FROM Sucursal WHERE nombre = ?");
            ps.setString(1, nombreSucursal); // esto hace matching con el signo de pregunta de la query
            resultado = ps.executeQuery();
    ...
    public ResultSet buscarFacturaPorId(Integer codigoId) {
        try {
            conectar();
            ps = conectar.prepareStatement("SELECT * FROM Sucursal WHERE nombre = ?");
            ps.setInt(1, codigoId); // misma consulta que buscarClientePorDNI....
            resultado = ps.executeQuery();
